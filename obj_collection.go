...
func (m *QdrantModule) collection() *object {
	return &object{
		impl: m,
		name: "collection",
		decl: collectionStates(),
		actions: map[string]action{
			"probed":    {validate: validateCollectionProbed, apply: (*QdrantModule).applyCollectionProbed},
			"present":   {validate: validateCollectionPresent, apply: (*QdrantModule).applyCollectionPresent},
			"recreated": {validate: validateCollectionRecreated, apply: (*QdrantModule).applyCollectionRecreated},
			"absent":    {validate: validateCollectionAbsent, apply: (*QdrantModule).applyCollectionAbsent},
		},
	}
}

// collectionDef is the object's entry in the artifact's bundle.
func collectionDef(m *QdrantModule) module.Def {
	return module.Def{
		Name:         "collection",
