...
func (m *QdrantModule) instance() *object {
	return &object{
		impl: m,
		name: "instance",
		decl: instanceStates(),
		actions: map[string]action{
			"pinged":         {validate: validateProbe, apply: (*QdrantModule).applyPinged},
			"ready-probed":   {validate: validateProbe, apply: (*QdrantModule).applyReadyProbed},
			"version-probed": {validate: validateProbe, apply: (*QdrantModule).applyVersionProbed},
		},
	}
}

// instanceDef is the object's entry in the artifact's bundle.
func instanceDef(m *QdrantModule) module.Def {
	return module.Def{
		Name:         "instance",
