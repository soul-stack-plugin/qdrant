...
func (m *Module) applyIndexPresent(ctx context.Context, stream eventStream, api qdrantAPI, params *structpb.Struct) error {
	f := params.GetFields()
...
func (m *Module) applyIndexAbsent(ctx context.Context, stream eventStream, api qdrantAPI, params *structpb.Struct) error {
	f := params.GetFields()
	apiKey := stringOrEmpty(f["api_key"])
	collection := strings.TrimSpace(stringOrEmpty(f["collection"]))
	field := strings.TrimSpace(stringOrEmpty(f["field"]))
...
func (m *Module) dropIndex(ctx context.Context, api qdrantAPI, collection, field, apiKey string) error {
