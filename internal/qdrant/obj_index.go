...
func (m *Module) index() *object {
	return &object{
		impl: m,
		name: "index",
		decl: indexStates(),
		actions: map[string]action{
			"present": {validate: validateIndexPresent, apply: (*Module).applyIndexPresent},
			"absent":  {validate: validateIndexAbsent, apply: (*Module).applyIndexAbsent},
		},
	}
}

// indexDef is the object's entry in the artifact's bundle.
func indexDef(m *Module) module.Def {
	return module.Def{
		Name:         "index",
