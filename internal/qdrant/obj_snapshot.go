...
func (m *Module) snapshot() *object {
	return &object{
		...
		actions: map[string]action{
			"created": {validate: validateSnapshotCreated, apply: (*Module).applySnapshotCreated},
			"absent":  {validate: validateSnapshotAbsent, apply: (*Module).applySnapshotAbsent},
		},
	}
}

// snapshotDef is the object's entry in the artifact's bundle.
func snapshotDef(m *Module) module.Def {
	return module.Def{
		Name:         "snapshot",
...
// already exists.
func (m *Module) applySnapshotCreated(ctx context.Context, stream eventStream, api qdrantAPI, params *structpb.Struct) error {
	f := params.GetFields()
...
func (m *Module) applySnapshotAbsent(ctx context.Context, stream eventStream, api qdrantAPI, params *structpb.Struct) error {
	f := params.GetFields()
	apiKey := stringOrEmpty(f["api_key"])
	collection := strings.TrimSpace(stringOrEmpty(f["collection"]))
