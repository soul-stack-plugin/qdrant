...
func (m *Module) alias() *object {
	return &object{
		impl: m,
		name: "alias",
		decl: aliasStates(),
		actions: map[string]action{
			"present": {validate: validateAliasPresent, apply: (*Module).applyAliasPresent},
			"absent":  {validate: validateAliasAbsent, apply: (*Module).applyAliasAbsent},
		},
	}
}

// aliasDef is the object's entry in the artifact's bundle.
func aliasDef(m *Module) module.Def {
	return module.Def{
		Name:         "alias",
